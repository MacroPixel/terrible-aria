...
# Crushes a 2D index [x, y] down to a 1D index [c]
def xy2c( x, y, w ):

  return ( y * w ) + x

# Converts a block's defining character into the character displayed in-game
def char2tile( c ):

  if c == ' ': return g_tmap[ 'air' ]
  if c == 'g': return g_tmap[ 'grass' ]
  if c == 's': return g_tmap[ 'stone' ]
  if c == 'w': return g_tmap[ 'log' ]
  if c == 'l': return g_tmap[ 'leaves' ]
  if c == 'i': return g_tmap[ 'iron' ]
  if c == 'v': return g_tmap[ 'silver' ]
  if c == 'a': return g_tmap[ 'gold' ]
...
def data_main_init():
  ...
  file.write( 'worlds: \n' )
  file.write( 'blocks: player=Δ;air= ;grass=~;stone=#;log=];leaves=*;iron=&;silver=$;gold=%' )
...
def generate_world( file ):
  ...
  for j in range( world_size.y ):
    for i in range( world_size.x ):
      g_tile_data += ' '
    progress = print_progress( progress, world_size.y, 5, 'Creating world array' ) # Update progress as it iterates
  g_tile_data = list( g_tile_data )

  # Fill in area under the noise curve
  for i in range( world_size.x ):
    t = int( ( world_size.y / 2 ) + noise_top( i, world_seed ) )

    # 4 blocks of grass
    for j in range( t, t + 4 ):
      g_tile_data[ xy2c( i, j, world_size.x ) ] = 'g'

    # And everything under it is stone
    for j in range( t + 4, world_size.y ):
      g_tile_data[ xy2c( i, j, world_size.x ) ] = 's'

    progress = print_progress( progress, world_size.x, 15, 'Generating initial terrain' ) # Update progress
  ...
  if ORES:
    ...
    for l1 in range( ORES_FREQ ): # Each iteration creates another vein
      ...
      o.y = random.randint( int( ( world_size.y / 2 ) + noise_top( o.x, world_seed ) ) + 10, world_size.y )

      d = 0 # This variable represents the direction (in radians)

      # Choose which kind of ore it is (depending on height)
      if ( o.y / world_size.y < 0.65 ):
        c = 'i'
      elif ( o.y / world_size.y < 0.75 ):
        c = random.choice( ( 'i', 'i', 'v' ) )
      else:
        c = random.choice( ( 'i', 'i', 'i', 'v', 'v', 'a' ) )
      ...
      for l2 in range( 48 ):
        ...
        o.a( cos( d ), sin( d ) )
        if o.y < ( world_size.y / 2 ) + noise_top( o.x, world_seed ) + 4:
          continue
  ...
  if TREES:
    ...
    while i1 < world_size.x - 15:
      ...
      o = V2( i1, int( ( world_size.y / 2 ) + noise_top( i1, world_seed ) - 1 ) )

      if g_tile_data[ xy2c( o.x, o.y + 1, world_size.x ) ] == 'g': # Make sure there's grass below

        # Continue upward for a random height [10, 20]
        for i2 in range( random.randint( 10, 20 ) ):

          # Place down a log and move vector upward
          g_tile_data[ xy2c( o.x, o.y, world_size.x ) ] = 'w'
          o.y -= 1
        ...
        for j in range( o.y - 4, o.y + 4 ):
          for i in range( o.x - 4, o.x + 4 ):
            if random.randint( 0, int( dist( V2( i, j ), o ) ) ) <= 1:
              g_tile_data[ xy2c( i, j, world_size.x ) ] = 'l'
  ...
  # Write buffer to world file
  for j in range( world_size.y ):
    file.write( ''.join( g_tile_data[ ( j * world_size.x ):( ( j + 1 ) * world_size.x ) ] ) )
    file.write( '\n' )
...
def data_world_init( name ):
  ...
  # Write non-tile details
  file.write( f'world_size: { world_size.x },{ world_size.y }\n')
  file.write( f'player_pos: { world_size.x // 2 },{ world_size.y // 2}\n' )
...
def data_world_load( name ):
  ...
  global g_pos, g_world_size, g_tile_data, g_show_help

  # Split file into statements
  file = open( 'w_' + name + '.txt', 'r' ).read().split( '\n' )

  # Read basic positional data
  g_world_size.x = int( file[0][12:].split( ',' )[0] )
  g_world_size.y = int( file[0][12:].split( ',' )[1] )
  g_pos.x = int( file[1][12:].split( ',' )[0] )
  g_pos.y = int( file[1][12:].split( ',' )[1] )

  # Read tile data
  g_tile_data = "";
  for j in range( g_world_size.y ):
    g_tile_data += file[2 + j]
  g_tile_data = list( g_tile_data )
...
def room_world_create():
  ...
  while True:
    ...
    # Create file and reload world list
    else:
        ...
        print( '[#] Invalid direction. (Accepts: "right", "left", "r", "l")')
          ...
          try:
            ...
            # Move in given direction and reload stage
              ...
              for i in range( t1 ):
                if g_tile_data[ xy2c( g_pos.x + ( 1 if p.split( ' ' )[1][0] == 'r' else -1 ), g_pos.y, g_world_size.x ) ] == ' ':
                  g_pos.x += ( 1 if t[0] == 'r' else -1 )
              goto_room( room_scene )
...
def run():
  ...
  # which isn't helpful if I want to be able to read a debug message.)
